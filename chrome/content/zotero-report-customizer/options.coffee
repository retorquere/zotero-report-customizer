...
initializePrefs = ->
  ...
  if itemTypes.childNodes.length is 0
    ...
    for type in Zotero.ReportCustomizer.fields().tree
      _type = elt(itemTypes, 'treeitem',
        container: 'true'
      )
      _type_row = elt(_type, 'treerow')
      _type_cell = elt(_type_row, 'treecell',
      ...
      for field in type.fields
        ...
        _field_cell = elt(_field_row, 'treecell',
    ...
    try
      ...
      order = (field in JSON.parse(Zotero.ReportCustomizer.prefs.getCharPref('sort')) when field.order && fields.indexOf(field.name) >= 0)
    catch err
      order = []
...
ReportSort_onDragOver = (event) ->
  ...
  false  if moved.indexOf('sortkey:') is 0
